...
def log_usage():
    ...
    custom_watt = data.get("custom_watt", None)   # ← user-supplied wattage
    user_id     = session["user_id"]

    # Use custom watt if provided, otherwise use default rating
    if custom_watt is not None:
        energy_kwh = round((float(custom_watt) / 1000) * hours, 4)
    else:
